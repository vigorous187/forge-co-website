...
function systemPrompt(brand, domain, vertical) {
  if (vertical === "forge-agency") {
    return `You write people-first marketing and local-visibility guides for ${brand} (${domain}), a GTA agency serving restaurants and local service businesses.
Output Markdown body only — no YAML frontmatter. Practical, specific to Greater Toronto Area restaurants where relevant.
Never claim guaranteed rankings, instant results, or "#1 on Google". Use "may", "often", "typically" for outcomes.
Include concrete examples operators can act on this week.`;
  }
  return `You write people-first blog posts for ${brand} (${domain}).
Output Markdown body only — no YAML frontmatter, no code fences wrapping the whole post.
Avoid hype and guaranteed outcomes.`;
}

/**
 * @param {{ slug: string, title: string, description?: string, tags?: string[] }} topic
 */
export async function generateBlogBody(topic) {
  ...
  try {
    ...
    site = {
      brand: "Forge Co.",
      domain: "forge-co.ca",
      conversionPath: "/growth-audit/",
      audience: "GTA restaurant owners",
      vertical: "forge-agency",
    };
  }
  const brand = process.env.BLOG_SITE_BRAND || site.brand;
  const domain = process.env.BLOG_SITE_DOMAIN || site.domain;
  const conversion = conversionPath(gates, site);
  const minWords = gates.blogQuality?.minWords ?? 900;
  const minH2 = gates.blogQuality?.minH2 ?? 4;
  const vertical = site.vertical || "forge-agency";
...
Title: ${topic.title}
Description hint: ${topic.description || topic.metaDescription || ""}
Tags: ${(topic.tags || []).join(", ")}
Audience: ${site.audience || "GTA restaurant owners"}
